    ...
    public class MarkdownRenderer
    {
        ...
        public virtual StringBuffer Render(IMarkdownRenderer render, MarkdownImageInlineToken token, MarkdownInlineContext context)
        {
            switch(token.LinkType)
            {
                ...
                default:
                    throw new NotSupportedException(string.Format(RS.UnsupportedLinkType, token.LinkType, "Image Link Render"));
           }
        }
        ...
        public virtual StringBuffer Render(IMarkdownRenderer render, MarkdownLinkInlineToken token, MarkdownInlineContext context)
        {
            switch (token.LinkType)
            {
                ...
                default:
                    throw new NotSupportedException(string.Format(RS.UnsupportedLinkType, token.LinkType, "Link Render"));
            }
        }
        ...
        private StringBuffer RenderNumberLink(IMarkdownRenderer render, IMarkdownToken token, MarkdownInlineContext context)
        {
            ...
            {
                throw new NotSupportedException(RS.BadNumberLinkToken);
            }
        ...
        private StringBuffer RenderRefLink(IMarkdownRenderer render, IMarkdownToken token, MarkdownInlineContext context)
        {
            ...
            {
                throw new NotSupportedException(RS.BadReferenceLinkToken);
            }
            ...
            foreach (var align in token.Align)
            {
                switch (align)
                {
                    ...
                    case Align.Center:
                        ...
                        break;
                    default:
                        throw new NotSupportedException(string.Format(RS.UnsupportedGfmTableAlignment, align));
                }
                content += "|";
            }
            content += "\n";
        ...
        public virtual StringBuffer Render(IMarkdownRenderer render, MarkdownListBlockToken token, MarkdownBlockContext context)
        {
            ...
            if (!token.Ordered)
            {
                ...
                foreach (var t in token.Tokens)
                {
                    ...
                    if (listItemToken == null)
                        throw new MarkdownException(string.Format(RS.BadMarkdownListItemBlockToken, t.GetType(), "Unordered", t.SourceInfo.Markdown));
            ...
            else
            {
                for (int i = 0; i < token.Tokens.Length; ++i)
                {
                    ...
                    if (listItemToken == null)
                        throw new MarkdownException(string.Format(RS.BadMarkdownListItemBlockToken, token.Tokens[i].GetType(), "Ordered", token.Tokens[i].SourceInfo.Markdown));
