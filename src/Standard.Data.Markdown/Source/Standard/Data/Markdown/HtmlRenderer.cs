...
namespace Standard.Data.Markdown
{
    ...
    public class HtmlRenderer
    {
        ...
        private static StringBuffer AppendSourceInfoCore(StringBuffer result, IMarkdownToken token)
        {
            return result + 
				" sourceFile=\"" + 
				StringHelper.HtmlEncode(token.SourceInfo.File) + 
				"\" sourceStartLineNumber=\"" + 
				token.SourceInfo.LineNumber.ToString() + 
				"\" sourceEndLineNumber=\"" + 
				(token.SourceInfo.LineNumber + token.SourceInfo.ValidLineCount - 1).ToString() + 
				"\"";
        }
