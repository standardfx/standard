...
namespace Standard.StringParsing
{
    partial class Parse
    {
        ...
        /// <summary>
        /// Creates a parser that returns the <see cref="ITextSpan{T}"/> of the parsed value.
        /// </summary>
        /// <typeparam name="T">The result type of the specified parser.</typeparam>
        /// <param name="parser">The parser to wrap.</param>
        /// <returns>
        /// A parser for the text span of the specified parser.
        /// </returns>
        public static Parser<ITextSpan<T>> Span<T>(this Parser<T> parser)
        {
            ...
            return i =>
            {
                ...
                if (r.WasSuccessful)
                {
                    TextSpan<T> span = new TextSpan<T>
                    {
                        Value = r.Value,
        ...
        /// <summary>
        /// Creates a parser that consumes a whitespace and all comments
        /// parsed by the <see cref="IComment.AnyComment"/> parser, but parses only one trailing
        ...
        /// <returns>
        /// An extended token version of the specified parser.
        /// </returns>
        public static Parser<ICommented<T>> Commented<T>(this Parser<T> parser, IComment commentParser = null)
        {
            ...
            Parser<string> comment = (commentParser ?? DefaultCommentParser).AnyComment;

            // parses any whitespace except for the new lines
            Parser<string> whiteSpaceExceptForNewLine = WhiteSpace.Except(Chars("\r\n")).Many().Text();
            ...
            Parser<ITextSpan<string>> commentSpan =
