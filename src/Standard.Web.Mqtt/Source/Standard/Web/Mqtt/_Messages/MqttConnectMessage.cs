...
{
	/// <summary>
	/// Message sent from client to broker for the purpose of requesting a connection to be established.
	/// </summary>
	/// <remarks>
	/// This is an implementation of the <c>CONNECT</c> message specification.
	/// </remarks>
	/// <seealso cref="MqttConnectAcknowledgeMessage"/>
	public class MqttConnectMessage : MqttMessage
    {
        ...
        /// <param name="keepAlivePeriod">Keep alive period</param>
        /// <param name="protocolVersion">Protocol version</param>
        public MqttConnectMessage(string clientId, string username, string password, bool willRetain, byte willQosLevel, bool willFlag, string willTopic, string willMessage, bool cleanSession, ushort keepAlivePeriod, byte protocolVersion)
        {
            this.type = MQTT_MSG_CONNECT_TYPE;
        ...
        public static MqttConnectMessage Parse(byte fixedHeaderFirstByte, byte protocolVersion, IMqttNetworkChannel channel)
        {
            byte[] buffer;
            int index = 0;
        ...
        public override byte[] GetBytes(byte protocolVersion)
        {
            ...
            if ((this.willQosLevel < MqttMessage.QOS_LEVEL_AT_MOST_ONCE) ||
                (this.willQosLevel > MqttMessage.QOS_LEVEL_EXACTLY_ONCE))
                throw new MqttClientException(MqttClientErrorCode.WillWrong);
