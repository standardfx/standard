...
namespace Standard.Win32.SafeHandles
{
   ...
   /// </summary>
   public abstract class SafeNativeMemoryBufferHandle : SafeHandleZeroOrMinusOneIsInvalid
   {
      ...
      public void CopyTo(byte[] destination, int destinationOffset, int length)
      {
         ...
         if (length > Capacity)
            throw new ArgumentOutOfRangeException("length", RS.SourceOffsetAndLengthOutsideBounds);
