...
namespace Standard.IO.Compression.LZ4.Tests
{
	public partial class Tools
	{
		...
		private static string FindRoot(string path = ".")
		{
			...
			while (true)
			{
				if (Directory.Exists(Path.Combine(path, "./testdeps")))
                    return Path.Combine(path, "./testdeps");

                if (IsRoot(path))
                    throw new DirectoryNotFoundException("Unable to find the 'testdeps' folder.");
