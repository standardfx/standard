...
namespace Standard.IO.Compression.LZ4.Tests
{
	public class ChecksumBlockTests
	{
		...
		public void HighlyCompressedFilesBinaryIdentical(string filename, int index, int length, int level, int expectedCompressedLength, uint expectedChecksum, string expectedBytes64)
		{
			var src = Tools.LoadChunk(Tools.FindFile(filename), index, length);

			var dst = LZ4CodecHelper.Encode(src, 0, src.Length, (LZ4CompressionLevel) level);
			var cmp = LZ4CodecHelper.Decode(dst, 0, dst.Length, src.Length);
