...
namespace Standard.IO.Compression.LZ4.Tests
{
	public class SpanTests
	{
		...
		[InlineData(1337, 1)]
		[InlineData(LZ4MemoryHelper.K64, LZ4MemoryHelper.K64)]
		public void CanEncodePartOfBuffer(int offset, int sourceLength)
		{
			...
			var encodedLength = LZ4Codec.Encode(source, offset, sourceLength, encoded, offset, targetLength);

			Check(encoded, 0, offset, 0xCD);
			Check(encoded, offset + encodedLength, offset, 0xCD);

			var decodedLength = LZ4Codec.Decode(encoded, offset, encodedLength, decoded, offset, sourceLength);
