...
namespace Standard.IO.Compression.LZ4.Tests
{
	public class StreamingRoundtripTests
	{
		...
		[InlineData("corpus/dickens", 0x10000 + 1, 0x10000, 8, 8)]
		public void TopupSize(string filename, int topupSize, int blockSize, int encoderExtraBlocks, int decoderExtraBlocks)
		{
		...
		public void ExtraBlocks(string filename, int topupSize, int blockSize, int encoderExtraBlocks, int decoderExtraBlocks)
		{
			Roundtrip(filename, topupSize, blockSize, encoderExtraBlocks, decoderExtraBlocks);
		}
		...
		public void BlockSize(string filename, int topupSize, int blockSize, int encoderExtraBlocks, int decoderExtraBlocks)
		{
			Roundtrip(filename, topupSize, blockSize, encoderExtraBlocks, decoderExtraBlocks);
		}
		...
		private static void Roundtrip(string filename, int topupSize, int blockSize, int encoderExtraBlocks, int decoderExtraBlocks)
		{
		...
		private static void RoundtripBlock(string filename, int topupSize, int blockSize, int decoderExtraBlocks)
		{
			var content = File.ReadAllBytes(Tools.FindFile(filename));
		...
		private static byte[] EncodeBlock(byte[] input, int topupSize, int blockSize)
		{
			...
			{
				using (var inputReader = new BinaryReader(inputStream, System.Text.Encoding.UTF8, false))
				using (var outputWriter = new BinaryWriter(outputStream, System.Text.Encoding.UTF8, true))
				using (var encoder = new LZ4BlockEncoder(LZ4CompressionLevel.Level0, blockSize))
				{
